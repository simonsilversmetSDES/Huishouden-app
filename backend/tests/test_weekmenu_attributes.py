...
@pytest.mark.parametrize(
    ("path", "model", "relation"),
    [
        ("moments", RecipeMoment, "moments"),
        ("categories", RecipeCategory, "categories"),
    ],
)
def test_delete_m2m_attribuut_in_gebruik_door_recept_geeft_409(
    logged_in: TestClient, seeded: Session, path: str, model: type, relation: str
) -> None:
    """Momenten en categorieën zijn many-to-many — koppelen via de relatie, niet via
    een simpele FK-kwarg zoals de andere attributen hierboven."""
    row = logged_in.get(f"{BASE}/{path}").json()[0]
    obj = seeded.get(model, row["id"])
    seeded.add(Recipe(title="Testrecept", **{relation: [obj]}))
    seeded.commit()
    resp = logged_in.delete(f"{BASE}/{path}/{row['id']}")
    assert resp.status_code == 409
    assert resp.json()["detail"]["code"] == "in_use"
